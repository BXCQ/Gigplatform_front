  ...
  <view class="detail-container">
    ...
    <view class="cu-custom">
      <view class="cu-bar fixed bg-gradient-green" style="height:90rpx;">
        ...
        <view class="content text-white">{{ pageTitle }}</view>
        <view class="action">
...
export default {
  data() {
    return {
      type: '', // 页面类型: waitOrder, selectedOrder, waitConfirm, completed
      jobList: []
    }
  },
  computed: {
    pageTitle() {
      ...
      return titleMap[this.type] || '个人用工'
    }
  },
  onLoad(options) {
    if (options.type) {
  ...
  methods: {
    ...
    goToDetail(job) {
      uni.navigateTo({
        url: `/pages/employer/personal/jobDetail?id=${job.id}`
      })
    },
